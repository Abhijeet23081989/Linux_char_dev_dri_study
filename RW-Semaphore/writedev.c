...
ssize_t writedev(struct file *myfile, const char __user *ubuff, size_t size, loff_t *offset)
{
	...
	noctw=lsize=devsize;
	//_______________RW_Semaphore Down read__________________________________
	printk(KERN_INFO"Before Down_write\n");
	printk(KERN_INFO"After Down_write\n");
	...
	ldev->datasize=nocsw;
	//___________________________RW-Release___________________________________
	up_write(&ldev->r1);
#ifdef DEBUG
