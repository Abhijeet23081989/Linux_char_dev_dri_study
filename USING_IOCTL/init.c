...
for(l=0;l<nod;l++)
{
	...
	dev[l].noofreg=noofreg=NOOFREG;//no of devices
	dev[l].regsize=regsize=REGSIZE;//register size
	dev[l].devsize=devsize=DEVSIZE;//device size
	//dev[l].datasize=datasize;//data size
	atomic_set(&dev[l].datasize,DATASIZE);
	//___________Semaphore Iniitialisation_____________________
	sema_init(&dev[l].s1,1);
	//__________Spinlock Initialization_______________________
	spin_lock_init(&dev[l].sl);
	devid=MKDEV(majorno,l);//for joining major and minor number
	printk(KERN_INFO"dev[l].regsize=%d\n",dev[l].regsize);
